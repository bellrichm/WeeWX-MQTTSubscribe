...
if [ "$CODECOVIO_UPLOAD" = "true" ]; then
  bash <(curl -s https://codecov.io/bash) -f coverage.xml -F unitests -n foo #>/dev/null
  bash <(curl -s https://codecov.io/bash) -f coverage2.xml -F integration -n bar #/dev/null
fi
