...
if [ "$CODECOVIO_UPLOAD" = "true" ]; then
  bash <(curl -s https://codecov.io/bash) -f nosetests.xml -F unitests -n unitests-$PYTHON #>/dev/null
  bash <(curl -s https://codecov.io/bash) -f nosetests2.xml -F integration -n integration-$PYTHON #/dev/null
fi
